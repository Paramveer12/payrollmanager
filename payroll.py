...
def editEmp():
    ...
    def new_Emp(): #Function to add new employee after admin password function
        ...
        emp_Details.append(new_id)
        
        new_Last = input("Last name ==> ")
        emp_Details.append(new_Last)
        
        new_First = input("First name ==> ")
        emp_Details.append(new_First)
        
        new_DOB = input("Birth date ==> ")
        emp_Details.append(new_DOB)
        
        new_Gender = input("Sex [M, F] ==> ")
        emp_Details.append(new_Gender)
        ...
        emp_Details.append(new_Depend)

        new_Med = input("Does the employee have medical allowance? [y or n] ==> ")
        emp_Details.append(new_Med)

        new_Hra = input("Does the employee have HRA? [y or n] ==> ")
        emp_Details.append(new_Hra)

        new_Hostel = input("Does the employee lives in hostel? [y or n] ==> ")
        emp_Details.append(new_Hostel)

        new_Bus = input("Does the employee have buss pass? [y or n] ==> ")
        emp_Details.append(new_Bus)
        ...
        def confirm_New(): #Function to double-check info above with user if it is correct
            ...
            if employee_Confirm == "y":
                with open("employee_details/employee_record.csv", mode = "a+")as add_Personal: #Opens personal .csv and assigns to variable
                    rec_adder = csv.writer(add_Personal, delimiter=",") #Writes info to .csv
                    rec_adder.writerow(emp_Details)
...
def reports():
    ...
    #Defining option #4
def payrollFunc():
    file1 = open("employee_details/employee_record.csv")
    
    record1 = csv.reader(file1)
    ...
    for item in record1:
        if item[0] == ("Serial Number"):
            ...
            position = item[5]
            payrate = item[6]
            file2 = open("Time_Cards/{}.csv".format(serial))
            records2 = csv.reader(file2)
            time = 0
            for item2 in records2:
                time = time + int(item2[2])
            ...
            if int(time) <= 40:
                ...
                try:
                    ...
                    f.write("\nPayrate -            $" + payrate)
                    f.write("\n\nHours worked -       " + str(time))
                    f.write("\n\nRegular Hrs. -       " + str(time))
                    f.write("\nOvertime Hrs.@ $0    0     $0")
                    f.write("\n\nTotal Pay - $" + str(gross))

                    if med_all == "y":
                        finalpay = round(float(gross + 39))
                        f.write("\n\nMedical Allowance:    $39")
